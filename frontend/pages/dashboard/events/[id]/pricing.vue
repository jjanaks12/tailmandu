...
onMounted(async () => {
    ...
    addons.value = data.addons.map((a: any) => ({
        ...a,
        applyToAll: a.applyToAll ?? a.apply_to_all,
        isMandatory: a.isMandatory ?? a.is_mandatory,
        specificStages: a.specificStages || []
    }))
        ...
        <div class="flex-grow min-w-0">
            <template v-if="trailRace">
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <h1 class="text-2xl font-bold font-headline">Pricing</h1>
                                        ...
                                        <div class="col-span-12 md:col-span-4 grid grid-cols-2 gap-2">
                                            ...
                                            <div v-if="tier.is_time_based">
                                                <Label :for="`end-date-${index}`"
                                                    ...
                                                    Date</Label>
                                                <Input :id="`end-date-${index}`" v-model="tier.endDate" type="date" />
                                            </div>
                                        </div>
                                        <div
                            ...
                            <CardHeader class="pb-4 border-b">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <Label :for="`override-${stage.id}`" class="text-sm cursor-pointer">
                                            Enable Custom Pricing
                                        </Label>
                                        <Switch :id="`override-${stage.id}`" v-model="stageOverrides[stage.id]" />
                                    </div>
                                </div>
                            </CardHeader>
                            <CardContent v-if="stageOverrides[stage.id]" class="pt-6">
                                <div class="space-y-4">
                                    <div v-for="(tier, index) in (stageSpecificTiers[stage.id] || [])" :key="tier.id"
                                        class="grid grid-cols-12 gap-4 items-end bg-gray-50/50 p-4 rounded-xl border">
                                        <div class="col-span-12 md:col-span-3">
                                            <Label class="text-xs mb-1.5 block text-muted-foreground">
                                                Tier Name
                                            </Label>
                                        ...
                                        <div class="col-span-12 md:col-span-3">
                                            ...
                                            <div class="relative">
                                                <span
                                                    class="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">Rs.</span>
                                                <Input v-model.number="tier.price" type="number" class="pl-10" />
                                            </div>
                                        </div>
                                        <div class="col-span-12 md:col-span-4 grid grid-cols-2 gap-2">
                                            <div class="col-span-2 flex items-center justify-end gap-2 mb-2">
                    ...
                    <TabsContent value="addons" class="space-y-6">
                        <Card>
                            ...
                            <CardContent>
                                    ...
                                    <div v-for="(addon, index) in addons" :key="addon.id"
                                        ...
                                        <div class="grid grid-cols-12 gap-4 items-end">
                                            ...
                                            <div class="col-span-12 md:col-span-3">
                                                ...
                                                <div class="relative">
                                                    <span
                                                        class="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground">Rs.</span>
                                                    <Input v-model.number="addon.price" type="number" class="pl-10" />
                                                </div>
                                            </div>
                                            <div class="col-span-12 md:col-span-3 flex items-center h-10 gap-2">
                                                <Switch :id="`mandatory-${index}`" v-model="addon.isMandatory" />
                                        ...
                                        <div class="border-t pt-4">
                                            <div class="flex items-center gap-2 mb-3">
                                                <Switch :id="`applyall-${index}`" v-model="addon.applyToAll" />
                                                <Label :for="`applyall-${index}`" class="text-sm cursor-pointer">Apply
                                                    to all stages</Label>
                                            </div>
                                            ...
                                            <div v-if="!addon.applyToAll"
                                                ...
                                                <div v-for="stage in stages" :key="stage.id"
                                                    ...
                                                    <Checkbox :id="`addon-${index}-stage-${stage.id}`"
                                                        :checked="addon.specificStages.includes(stage.id)"
                                                        @update:checked="$event ? addon.specificStages.push(stage.id) : addon.specificStages = addon.specificStages.filter((s: any) => s !== stage.id)" />
                                ...
                                <Button modifier="outline" @click="addAddon" class="mt-6 w-full border-dashed gap-2">
