...
export function useGpxData() {
    ...
    const parseGpxText = (gpxText: string, computeElevation = true) => {
        const parser = new DOMParser()
        const gpxDoc = parser.parseFromString(gpxText, 'application/xml')
        const trkpts = Array.from(gpxDoc.querySelectorAll('trkpt'))

        if (trkpts.length > 0) {
            // Always set start and end points for markers
            const firstPt = trkpts[0]
            const lastPt = trkpts[trkpts.length - 1]
            startPoint.value = [0, 0, parseFloat(firstPt.getAttribute('lat') || '0'), parseFloat(firstPt.getAttribute('lon') || '0')]
            endPoint.value = [0, 0, parseFloat(lastPt.getAttribute('lat') || '0'), parseFloat(lastPt.getAttribute('lon') || '0')]

            // Always extract basic waypoints for markers
            ...
            if (computeElevation) {
                ...
                // Overwrite with accurate data when computing elevation
                ...
                wptList.forEach(wpt => {
                    ...
                    for (let pt of points) {
                        const d = L.latLng(wpt.lat, wpt.lng).distanceTo(L.latLng(pt[2], pt[3]))
                        if (d < minWptDiff) {
                            minWptDiff = d
                            closestDist = pt[0] // distance in km
                        }
                    }
                    wpt.dist = closestDist
                })
            }
